...
-- Data Analysis
-- Below I provided the SQL Codes to find the answers of the 16 questions provided in the Problem Statement.
--------------------------------------------------------------------------------------------
...
5. I used ROUND() function to give us a rounded figure of the Average values.
...
/*
In this query:
1. I calculated the summary statistics for Confirmed Corona Cases column. I calculated the total number of cases, average, minimum, median, maximum value, the variance and standard deviation.
...
/*
In this query:
1. I calculated the summary statistics for Death Corona Cases column. I calculated the total number of cases, average, minimum, median, maximum value, the variance and standard deviation.
...
/*
In this query:
1. I calculated the summary statistics for Recovered Corona Cases column. I calculated the total number of cases, average, minimum, median, maximum value, the variance and standard deviation.
...
-- Ratio of Death vs Confirmed corona cases 
SELECT 
	ROUND(SUM([Deaths])/SUM([Confirmed]),4) AS Death_Vs_Confirmed_Ratio
FROM
	[dbo].[CoronaVirusData]
HAVING
	SUM([Confirmed]) != 0;
...
--Ratio of Recovered vs Confirmed corona cases

SELECT 
	ROUND(SUM([Recovered])/SUM([Confirmed]),4) AS Recovered_Vs_Confirmed_Ratio
FROM
	[dbo].[CoronaVirusData]
HAVING
	SUM([Confirmed]) != 0;

-- Ratio of Death vs Confirmed corona cases per country

SELECT 
	[Country/Region], 
	ROUND(SUM([Deaths])/SUM([Confirmed]),2) AS Death_Vs_Confirmed_Ratio
FROM
	[dbo].[CoronaVirusData]
GROUP BY
	[Country/Region]
...
-- Ratio of Recovered vs Confirmed corona cases per country

SELECT 
	[Country/Region], 
	ROUND(SUM([Recovered])/SUM([Confirmed]),2) AS Recovered_Vs_Confirmed_Ratio
FROM
	[dbo].[CoronaVirusData]
GROUP BY
	[Country/Region]
...
-- 10 countries with lowest Confirmed cases
...
-- 10 countries with lowest death cases
SELECT 
	TOP 10 [Country/Region], 
	SUM([Deaths]) AS Death_Cases
FROM 
...
-- 10 countries with highest death cases
